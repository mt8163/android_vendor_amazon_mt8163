...
inline String8 getSessionModeString(DISP_MODE mode)
{
    ...
    buffer_handle_t handle, PrivateHandle* priv_handle)
{
    ...
    else
    {
        ...
        if (format == HAL_PIXEL_FORMAT_NV12_BLK  || format == HAL_PIXEL_FORMAT_UFO
...
{
    if ((priv_handle->usage & GRALLOC1_CONSUMER_USAGE_GPU_TEXTURE) &&
...
inline bool isUIPq(PrivateHandle* priv_handle)
{
