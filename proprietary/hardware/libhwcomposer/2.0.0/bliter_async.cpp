...
void AsyncBliterHandler::set(const sp<HWCDisplay>& display, DispatcherJob* job)
{
    ...
    for (uint32_t i = 0; i < total_num; i++)
    {
        ...
        if (hw_layer->dirty)
        {
            ...
            logger.printf("/rel=%d/acq=%d/handle=%p/job=%d",
                layer->getReleaseFenceFd(), layer->getAcquireFenceFd(), layer->getHandle(), hw_layer->mdp_job_id);
    ...
    DispatcherJob* job)
{
    ...
    for (uint32_t i = 0; i < total_num; i++)
    {
        ...
        if (HWC_LAYER_TYPE_MM_FBT != hw_layer->type)
        {
            getPrivateHandleBuff(layer->handle, priv_handle);
        }
        else
        {
        ...
        {
            // get release fence from mdp
            m_blit_stream.createJob(hw_layer->mdp_job_id, layer->releaseFenceFd);

            memcpy(&hw_layer->layer, layer, sizeof(hwc_layer_1_t));
            hw_layer->layer.releaseFenceFd = dup(layer->releaseFenceFd);

            logger.printf("/rel=%d/acq=%d/handle=%p/job=%d",
                layer->releaseFenceFd, layer->acquireFenceFd, layer->handle, hw_layer->mdp_job_id);
        ...
        else
        {
            ...
            logger.printf("/async=bypass/acq=%d/handle=%p", layer->acquireFenceFd, layer->handle);
    ...
    DispatcherJob* job)
{
    ...
    {
        BLOGE(0, "setMirror, list is NULL");
        return;
    }
...
void AsyncBliterHandler::process(DispatcherJob* job)
{
    ...
    for (uint32_t i = 0; i < total_num; i++)
    {
        ...
        sp<DisplayBufferQueue> queue = getDisplayBufferQueue(&hw_layer->priv_handle, config, i);

        status_t err = setDpConfig(&hw_layer->priv_handle, config, i);
        ...
        if (NO_ERROR == err)
        {
            ...
            bool pq_enhance = (HWCMediator::getInstance().m_features.is_support_pq) &&
            ...
            {
                WDT_BL_NODE(setSrc, config, hw_layer->priv_handle, &layer->acquireFenceFd);
                WDT_BL_NODE(setDst, 0, &param, disp_buffer.out_ion_fd,
                        disp_buffer.out_sec_handle,
                        &disp_buffer.release_fence);
                WDT_BL_NODE(invalidate, hw_layer->mdp_job_id, &mdp_cal_dst_crop);
                ...
                uint32_t* xform, Rect* src_roi, Rect* dst_roi, DispatcherJob* job)
{
    ...
    if (1 == HWCMediator::getInstance().getOvlDevice(job->disp_mir_id)->getCapsInfo()->is_output_rotated)
    {
        ori_rot = (ori_rot + 2) % 4;
    }
