...
void AsyncBlitDevice::getSupportedResolution(KeyedVector<int, OvlInSizeArbitrator::InputSize>& /*config*/)
{
}
...
status_t AsyncBlitDevice::createOverlaySession(int dpy, DISP_MODE mode)
{
...
status_t AsyncBlitDevice::setOverlaySessionMode(int dpy, DISP_MODE mode)
{
    checkValidDpyRetVal(dpy, INVALID_OPERATION);
...
DISP_MODE AsyncBlitDevice::getOverlaySessionMode(int dpy)
{
...
status_t AsyncBlitDevice::getOverlaySessionInfo(int dpy, disp_session_info* info)
{
    checkValidDpyRetVal(dpy, INVALID_OPERATION);
...
disp_caps_info* AsyncBlitDevice::getCapsInfo()
{
    return &m_caps_info;
}

bool AsyncBlitDevice::queryValidLayer(disp_layer_info* /*disp_layer*/)
{
    HWC_LOGE("Do NOT call %s", __func__);
    return false;
}
