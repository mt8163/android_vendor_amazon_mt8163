    ...
    , m_ultra_scenario(false)
{
    ...
    property_get("vendor.debug.hwc.is_unique_composer", value, "1");
        ...
        else
        {
            ...
            if (job->num_layers <= 0)
            {
                ...
                if (dpy < HWC_DISPLAY_VIRTUAL ||
                    m_workers[HWC_DISPLAY_PRIMARY].ovl_engine->getOverlaySessionMode() != DISP_SESSION_DECOUPLE_MIRROR_MODE)
                {
            ...
            DISP_MODE display_session_mode = m_workers[dpy].ovl_engine->getOverlaySessionMode();
            if (display_session_mode == DISP_SESSION_DECOUPLE_MIRROR_MODE ||
                display_session_mode == DISP_SESSION_DIRECT_LINK_MIRROR_MODE) {
...
bool HWCDispatcher::decideDirtyAndFlush(
    ...
    const int64_t& hwc2_layer_id, const int32_t& layer_caps)
{
    if (idx >= static_cast<int32_t>(m_workers[dpy].prev_hw_layers.size()))
    {
        return true;
    ...
    HWC_LOGV("decideDirtyAndFlush() ret:%d type:%d(%d) pool_id:%d(%d), layer_id:%" PRId64 "(%" PRId64")",
        ret, type, prev_hw_layer.type, pool_id, prev_hw_layer.pool_id, hwc2_layer_id, prev_hw_layer.hwc2_layer_id);
...
DISP_MODE HWCDispatcher::getSessionMode(const int& dpy)
{
...
void HWCDispatcher::setJob(const sp<HWCDisplay>& display)
{
...

    {
        ...
        {
            display->clearAllFences();
            HWC_LOGW("(%" PRIu64 ") SET/bypass/no_visible_layers", display->getId());
            return;
        }
        ...
        if (job->fbt_exist)
        {
            ...
            if (fbt_layer->getHandle() == NULL)
            {
                ...
                if (0 == (job->num_mm_layers + job->num_ui_layers))
                {
                    display->clearAllFences();
                    job->enable = false;
                    HWC_LOGD("(%" PRIu64 ") SET/bypass/no layers", dpy);
                    return;
                }
            }
        }
        ...
        logger->printf("(%" PRIu64 ") SET job=%d/max=%d/fbt=%d(%s)/ui=%d/mm=%d/del=%d/mir=%d/ult=%d/flush=%d/black=%d",
            ...
            if (job->num_mm_layers)
            {
                if (HWC_MIRROR_SOURCE_INVALID != job->disp_mir_id)
                {
                    ...
                    if ((mir_job->post_state & HWC_POST_CONTINUE_MASK) != 0)
                    {
                        ...
                        if (mir_out_acq_fence_fd != -1) ::protectedClose(mir_out_acq_fence_fd);
                    }
                }
            }
            // disable job
        ...
        if (job->num_ui_layers || (job->fbt_exist && !job->mm_fbt))
        {
            if (!m_is_worker_all_in_one)
                m_workers[dpy].ui_thread->set(display, job);
        ...
        if (job->num_mm_layers || job->mm_fbt)
        {
            ...
            {
                if (!m_is_worker_all_in_one)
                    m_workers[dpy].mm_thread->set(display, job);
...
void HWCDispatcher::releaseResourceLocked(int dpy)
{
    ...
    m_workers[dpy].ovl_engine->trigger(true, 0, DISP_NO_PRESENT_FENCE, -1, false, nullptr, TRIGGER_NORMAL, true);
    ...
    const sp<HWCDisplay>& display, DispatcherJob* job)
{
    ...
    if (1 == HWCMediator::getInstance().m_features.svp)
    {
        ...
        err = handleSecureBuffer(job->secure, outbuf_hnd, &priv_handle->sec_handle, &priv_handle->ext_info);
        if (NO_ERROR != err)
        {
            PLOGE("alloc sec outbuf fail!! (err=%d, h=%x)", err, outbuf_hnd);
            job->post_state = HWC_POST_INVALID;
            display->clearAllFences();
            setError(job);
            return;
