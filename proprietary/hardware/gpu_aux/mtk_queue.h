...
public:
    GPUAUXBuffer(uint32_t width,
                 uint32_t height,
                 uint32_t format) : BASE(), mOwner(ownHandle), mWrappedBuffer(NULL)
    {
    ...
    {
        if (mOwner == ownHandle) {
            MTKGralloc::getInstance()->freeBuffer(static_cast<android_native_buffer_t *>(const_cast<GPUAUXBuffer *>(this)));
        }
        mWrappedBuffer = NULL;
    }
