...
//#endif
    ...
    property_get("vendor.mtk.omxvdec.avpf", value, "1");
                                         ...
                                         OMX_IN OMX_U32 nSizeBytes)
{
    ...
    {
        if (mTeeType == TRUSTONIC_TEE || mTeeType == BLOWFISH_TEE)
        {
                int fd = -1;
                fd = open("/proc/svp_region", O_RDONLY);
...
OMX_BOOL MtkOmxVdec::ConvertFrameToYUV420(FrmBufStruct *pFrameBuf, FrmBufStruct *pFrameBufOut, OMX_BOOL bGetResolution)
{
    ...
    MTK_OMX_LOGD("@Invalidate Cache After MDP");
    eVideoInvalidateCache(NULL, 0, 0);
...
{
    if (OMX_TRUE == needColorConvertWithMetaMode() || OMX_TRUE == needColorConvertWithoutMetaMode() || OMX_TRUE == mNativeWindowHDRInternalConvert)
    {
        return OMX_TRUE;
    }
    return OMX_FALSE;
...
void MtkOmxVdec::SetInputPortViLTE()
{
    if (mCodecId == MTK_VDEC_CODEC_ID_AVC)
    {
        ...
        mInputPortDef.nBufferSize = MTK_VDEC_AVC_DEFAULT_INPUT_BUFFER_SIZE_VILTE;
    }
}
