...
// m4u map [
struct OmxM4uMVAEntry : public OmxMVAEntry
{
        OmxM4uMVAEntry() {}
        virtual ~OmxM4uMVAEntry() {}

    private:
        OmxM4uMVAEntry(const OmxM4uMVAEntry &);
        OmxM4uMVAEntry &operator=(const OmxM4uMVAEntry &);
};

struct OmxM4uMVAMap : public OmxMVAMap
{
        OmxM4uMVAMap();
        virtual ~OmxM4uMVAMap();

        virtual int setBoolProperty(const char *key, OMX_BOOL value);
        //virtual int setCharProperty(const char *key, OMX_BOOL value);
        virtual void *newEntry(int align, int size, void *bufHdr);
        virtual void *newEntryWithVa(void *va, int size, void *bufHdr);
        virtual void *newEntryWithHndl(void *handle, void *bufHdr);
        virtual void freeEntry(void *entry);
    ...
    private:
        ...
        static int mM4UHandleCount;

        OMX_BOOL    mIsMCIMode;

        OmxM4uMVAEntry *mapM4u(void *va, int size, void *bufHdr);

        OmxM4uMVAMap(const OmxM4uMVAMap &);
        OmxM4uMVAMap &operator=(const OmxM4uMVAMap &);
};
// ] (m4u map)
