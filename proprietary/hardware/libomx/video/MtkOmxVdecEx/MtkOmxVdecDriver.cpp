...
OMX_BOOL MtkOmxVdec::InitHEVCSecParams() //HEVC.SEC.M0
{
    ...
    if (svp)
    {
        ...
        {
            property_get("ro.vendor.mtk_blowfish_tee_support", value, "0");
            if (atoi(value))
            {
                svp_mode = 3;
        ...
        property_set("vendor.mtk_sec_enable", "svp_mode");
...
OMX_BOOL MtkOmxVdec::InitVp9SecParams()
{
    ...
    if (svp_mode == 0)
    {
        MTK_OMX_LOGE("[ERROR] InitVP9SecParams, check Tee Type Failed !!");
        return OMX_FALSE;
    }
