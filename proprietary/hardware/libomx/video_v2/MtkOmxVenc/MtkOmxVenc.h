...
#define MTK_VENC_DEFAULT_OUTPUT_BUFFER_SIZE_AVC_1080P (2*1024*1024)
...
class MtkOmxVenc : public MtkOmxBase
{
    ...
    private:
        ...
        //void echoChipName(VAL_UINT32_T u4ChipName);
        ...
        int ShiftUVforAndroidYV12(unsigned char *dst, int dstWidth, int dstHeight);
    ...
    VAL_UINT32_T Dump_buf_sec_and_normal(OMX_U32 hSec_buf, OMX_U8* pTemp_Buf,OMX_U32 ui4_sz, unsigned  dir);
        ...
        //const char* indexType(OMX_INDEXTYPE);
        ...
        int ioctl_q_in_buf(int idx, int dmabuf_fd);
        int ioctl_q_out_buf(int idx, int dmabuf_fd);
        int ioctl_dq_in_buf(int *pdq_in_buf_idx);
        int ioctl_dq_out_buf(int *pdq_out_buf_idx, int *bs_size, int *pflags);
        int IoctlPoll();
        int ioctl_stream_on();
        int ioctl_stream_off();
        int ioctl_runtime_config(int change_to_mode, int change_to_value);
