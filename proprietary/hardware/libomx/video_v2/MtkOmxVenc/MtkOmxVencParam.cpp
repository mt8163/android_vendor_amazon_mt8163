...
OMX_ERRORTYPE MtkOmxVenc::SetInputPortDefinition(OMX_PARAM_PORTDEFINITIONTYPE *pPortDef)
{
    ...
    MTK_OMX_LOGD_ENG("@@ Set input port: nFrameWidth(%d), nFrameHeight(%d), nStride(%d), nSliceHeight(%d), "
    ...
    {
        watchdogTimeout = (mInputPortDef.format.video.xFramerate >> 16)*10;
        MTK_OMX_LOGD_ENG("betfor setprop watchdogTimeout is %d",watchdogTimeout);

        int setValue = MtkVenc::WatchdogTimeout();
        MTK_OMX_LOGD_ENG("setprop setValue to %d",setValue);

        if(setValue != 0 || watchdogTimeout < setValue){
...
OMX_ERRORTYPE MtkOmxVenc::SetOutputPortDefinition(OMX_PARAM_PORTDEFINITIONTYPE *pPortDef)
{
    ...
    MTK_OMX_LOGD_ENG("@@ Set output port: nFrameWidth(%d), nFrameHeight(%d), nStride(%d), nSliceHeight(%d),"
...
OMX_ERRORTYPE MtkOmxVenc::HandleSetVideoBitrate(OMX_VIDEO_PARAM_BITRATETYPE* pBitrateType)
{
    ...
    MTK_OMX_LOGD_ENG("nTargetBitrate=%d, mode=%x", (int)pBitrateType->nTargetBitrate,
...
OMX_ERRORTYPE MtkOmxVenc::HandleSetVendorSetScenario(OMX_PARAM_U32TYPE* pSetScenarioInfo)
{
    ...
    if (tmpVal == OMX_VIDEO_MTKSpecificScenario_ViLTE) { mIsViLTE = true; }
...
// to-do: break down, QueryVideoProfileLevel
