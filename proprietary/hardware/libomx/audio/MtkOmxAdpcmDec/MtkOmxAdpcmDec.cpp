        ...
        if(OMX_FALSE == mInitFlag) {
            if(initAdpcmDecoder(pInputBuf)) {
                ...
                if((mOutputPcmMode.nChannels != mInputAdpcmParam.nChannelCount) || (mOutputPcmMode.nSamplingRate != mInputAdpcmParam.nSamplesPerSec)) {
                    ...
                    SLOGV("ADPCM init success and port format is ok ,queue input and output buffer !!!");
            ...
            if(fgFlush == OMX_TRUE) {
                ...
                SLOGV("[FLACDec]push the input buffer back to the head of the input queue!");
...
void MtkOmxAdpcmDec :: handleFormatChanged(OMX_BUFFERHEADERTYPE* pInputBuffer,OMX_BUFFERHEADERTYPE* pOutputBuffer)
{
    ...
    HandleFillBufferDone(pOutputBuffer);
    QueueInputBuffer(findBufferHeaderIndex(MTK_OMX_INPUT_PORT, pInputBuffer));
}
