...
enum DREEvent {
    ...
    eEvtConfigChanged   = 0x10,
    // Some configuration field has been written
    eEvtFieldWritten    = 0x20,
...
struct DRETopInput {
    ...
    int DREChromaHist[mdpDRE_BLK_NUM_Y][mdpDRE_BLK_NUM_X];
...
private:
  ...
  void G_AdaptiveLuma_Block( int dre_blk_x_num, int dre_blk_y_num, int HistNorEn, int *HisBin, int RGBMaxSum, int ChromaHistCount, int *iYFtn );
  void L_AdaptiveLuma_Block( int dre_blk_x_num, int dre_blk_y_num, int HistNorEn, int *HisBin, int RGBMaxSum, int ChromaHistCount, int *iYFtn );
