...
void UncaligyroSensor::processEvent(int type, int code, int value) {
    /* ALOGD("processEvent code=%d,value=%d\r\n",code, value); */
    if (EV_ABS == type) {
        switch (code) {
            case EVENT_TYPE_UNCALI_GYRO_X:
                mPendingEvent.uncalibrated_gyro.x_uncalib = (float)value / mDataDiv;
                break;
            case EVENT_TYPE_UNCALI_GYRO_Y:
                mPendingEvent.uncalibrated_gyro.y_uncalib = (float)value / mDataDiv;
                break;
            case EVENT_TYPE_UNCALI_GYRO_Z:
                mPendingEvent.uncalibrated_gyro.z_uncalib = (float)value / mDataDiv;
                break;
            case EVENT_TYPE_UNCALI_GYRO_X_BIAS:
                mPendingEvent.uncalibrated_gyro.x_bias = (float)value / mDataDiv;
                break;
            case EVENT_TYPE_UNCALI_GYRO_Y_BIAS:
                mPendingEvent.uncalibrated_gyro.y_bias = (float)value / mDataDiv;
                break;
            case EVENT_TYPE_UNCALI_GYRO_Z_BIAS:
                mPendingEvent.uncalibrated_gyro.z_bias = (float)value / mDataDiv;
                break;
        }
    } else if (EV_REL == type) {
        switch (code) {
            case EVENT_TYPE_UNCALI_GYRO_TIMESTAMP_HI:
                mPendingEvent.timestamp = (mPendingEvent.timestamp & 0xFFFFFFFFLL) | ((int64_t)value << 32);
                break;
            case EVENT_TYPE_UNCALI_GYRO_TIMESTAMP_LO:
