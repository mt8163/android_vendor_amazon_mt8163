...
int WakeUpSetSensor::enable(int32_t handle, int en) {
    ...
    if (nonWakeUpHandle < 0)
        return 0;
    mSensorManager->activate(mSensorConnection, nonWakeUpHandle, en);
    return 0;
}
...
int WakeUpSetSensor::batch(int handle, int flags,
    ...
    if (nonWakeUpHandle < 0)
        return 0;
    mSensorManager->batch(mSensorConnection, nonWakeUpHandle, samplingPeriodNs, maxBatchReportLatencyNs);
    return 0;
}
