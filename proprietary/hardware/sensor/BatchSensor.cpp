...
int BatchSensor::enable(int32_t handle, int en)
{
    ...
    if (en == 1)
        mEnabledTime[handle] = getTimestamp() + IGNORE_EVENT_TIME;
            ...
            for (i=0;count&&i<sensors_data.numOfDataReturn;i++)
            {
                ...
                }else
                {
                    if (data->sensor >= 0 && data->sensor < numSensors && mDataDiv[data->sensor] != 0) {
                        ...
                        ALOGE("Dropped data because sensortype(%d) > numSensors or div(%d) == 0\n",
                            data->sensor, mDataDiv[data->sensor]);
                    }
                }
                if (data->timestamp > mEnabledTime[data->sensor]) {
                    data++;
                    count--;
                    numEventReceived++;
                } else {
                    ALOGE("Dropped data because timestamp(%lld) < mEnabledTime\n", data->timestamp);
                }
