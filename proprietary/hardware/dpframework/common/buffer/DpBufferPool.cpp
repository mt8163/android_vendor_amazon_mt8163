    ...
    for(iterator = m_bufferList.begin(); iterator != m_bufferList.end(); iterator++)
    {
        ...
        if(m_bufferList.size() == 1)
        {
            snprintf(bufferInfoStr, strLen,
                    "Buffer info: plane=%d bufVA={%p,%p,%p} bufMVA={0x%x,0x%x,0x%x} bufSize={%d,%d,%d}",
                    iterator->plane,
        ...
        DPLOGD("Buffer info: plane=%d bufVA={%p,%p,%p} bufMVA={0x%x,0x%x,0x%x} bufSize={%d,%d,%d}\n",
                iterator->plane,
                pVABase[0], pVABase[1], pVABase[2],
                mvaBase[0], mvaBase[1], mvaBase[2],
                bufSize[0], bufSize[1], bufSize[2]);
                                            ...
                                            uint32_t      jobID)
{
    ...
    sprintf(name, "%s%05d_%s_job%05u.bmp", DUMP_ROOT, current_tid, keyword, jobID);
    ...
    sprintf(name, "%s%05d_%s_job%05u.bin", DUMP_ROOT, current_tid, keyword, jobID);
    DPLOGD("DpBufferPoolBase: dump file %s\n", name);
        ...
        if (false == iterator->flush)
        {
            if (NULL != m_pProxy[0])
            {
                m_pProxy[0]->flushMemory(DpMemory::FLUSH_BEFORE_HW_READ_BUFFER_RANGE,
                                         (uint32_t)(iterator->pVABase[0]) - (uint32_t)m_pRingVA[0],
                                         iterator->size[0]);

                if (DP_COLOR_GET_PLANE_COUNT(m_bufferFormat) >= 2)
                {
                    m_pProxy[0]->flushMemory(DpMemory::FLUSH_BEFORE_HW_READ_BUFFER_RANGE,
                                             (uint32_t)(iterator->pVABase[1]) - (uint32_t)m_pRingVA[0],
                                             iterator->size[1]);
                }
                ...
                {
                    m_pProxy[0]->flushMemory(DpMemory::FLUSH_BEFORE_HW_READ_BUFFER_RANGE,
                                             (uint32_t)(iterator->pVABase[2]) - (uint32_t)m_pRingVA[0],
                                             iterator->size[2]);
                }
            }
            iterator->flush = true;
