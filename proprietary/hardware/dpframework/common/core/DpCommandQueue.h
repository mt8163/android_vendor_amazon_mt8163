...
public:
    ...
    virtual void getReadbackStatus(bool& pq_readback, bool& hdr_readback, int32_t& dre_readback, uint32_t& engineFlag, uint32_t& VEncFlag);

    virtual bool getSyncMode();

    virtual uint32_t* getReadbackPABuffer(uint32_t& readbackPABufferIndex);

    virtual DP_STATUS_ENUM setNumReadbackPABuffer(uint32_t numReadbackPABuffer);
