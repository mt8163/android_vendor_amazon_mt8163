...
public:
    ...
    virtual DP_STATUS_ENUM writeFromMem(uint32_t addr,
                                        uint32_t value,
                                        uint32_t mask = 0xFFFFFFFF) = 0;
    ...
    virtual void getReadbackStatus(bool& pq_readback, bool& hdr_readback, int32_t& dre_readback, uint32_t& engineFlag, uint32_t& VEncFlag) = 0;

    virtual bool getSyncMode() = 0;

    virtual uint32_t* getReadbackPABuffer(uint32_t& readbackPABufferIndex) = 0;

    virtual DP_STATUS_ENUM setNumReadbackPABuffer(uint32_t numReadbackPABuffer) = 0;
