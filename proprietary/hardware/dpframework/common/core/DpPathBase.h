...
public:
    ...
    DP_STATUS_ENUM getPathFlag(int32_t *pFlag)
    {
        if (NULL == pFlag)
        {
            return DP_STATUS_INVALID_PARAX;
    ...
    {
        return onSetNumReadbackPABuffer(numReadbackPABuffer);
    }
...
protected:
    ...
    virtual DP_STATUS_ENUM onGetPathFlag(int32_t *pFlag) = 0;
    ...
    virtual DP_STATUS_ENUM onSetNumReadbackPABuffer(uint32_t numReadbackPABuffer) = 0;
};
