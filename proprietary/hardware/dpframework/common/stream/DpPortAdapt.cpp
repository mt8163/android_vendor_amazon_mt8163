...
DP_STATUS_ENUM DpPortAdapt::dumpBuffer(CLIENT_TYPE_ENUM client)
{
    ...
    m_pPortPool->getSWBaseWithID(bufID,
                                 pBase,
                                 size);
    ...
    if (0 == DP_COLOR_GET_BLOCK_MODE(m_portFormat))
    {
        width = (m_portYPitch << 3) / DP_COLOR_BITS_PER_PIXEL(m_portFormat);

        if (size[0] != uint32_t(m_portHeight * m_portYPitch))
        {
            height = size[0] / m_portYPitch;
        }
    }
    ...
    DPLOGD("DpPortAdapt: dump buffer format %#010x, width %d, height %d, Y pitch %d, UV pitch %d\n", m_portFormat, width, height, m_portYPitch, m_portUVPitch);

    sprintf(name, "%s%05d_%05d_%02d-%s.bmp", DUMP_ROOT, current_tid, m_dumpID, m_dumpIndex, CLIENT_PRODUCER == client ? "output" : "input");
    DPLOGD("DpPortAdapt: dump file %s\n", name);
    ...
    sprintf(name, "%s%05d_%05d_%02d-%s.bin", DUMP_ROOT, current_tid, m_dumpID, m_dumpIndex, CLIENT_PRODUCER == client ? "output" : "input");
    DPLOGD("DpPortAdapt: dump file %s\n", name);
