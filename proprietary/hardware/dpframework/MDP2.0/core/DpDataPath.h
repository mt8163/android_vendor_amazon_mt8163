...
private:
    ...
    virtual DP_STATUS_ENUM onGetPathFlag(int32_t *pFlag)
    {
        int32_t index;
    ...
    {
        return configPMQOS(pPMQOS);
    }
    DP_STATUS_ENUM configPMQOS(mdp_pmqos *pPMQOS)
    {
        Engine  *pCurr;
        int32_t index;
        ...
        for (index = 0; index < m_rootAndLeaf; index++)
        {
            ...
            {
                pCurr->updatePMQOS(totalPixel, mdpDataSize, end_Time);
                pPMQOS->mdp_total_datasize = mdpDataSize; //Uint: byte
                pPMQOS->mdp_total_pixel = totalPixel; //Uint: pixel
            }
            else
            {
                //tIMGI or tIMG2O
                pCurr->updatePMQOS(totalPixel, ispDataSize, end_Time);
                pPMQOS->isp_total_datasize = ispDataSize; //Uint: byte
                pPMQOS->isp_total_pixel = totalPixel; //Uint: pixel
            }
    ...
    virtual DP_STATUS_ENUM onSetNumReadbackPABuffer(uint32_t numReadbackPABuffer)
    {
        if(NULL == m_pStream)
        {
        ...
        return m_pStream->setNumReadbackPABuffer(numReadbackPABuffer);
    }
};
