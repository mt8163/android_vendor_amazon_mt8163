...
inline DP_STATUS_ENUM DpEngineBase::configFrameRead(DpCommand &command, DpConfig &config)
{
    ...
    size[0] = pitch_y * height;
    size[1] = (plane_num > 1) ? (pitch_uv * height_uv) : 0;
    size[2] = (plane_num > 2) ? (pitch_uv * height_uv) : 0;
    ...
    config.inYPitch   = pitch_y;
    config.inUVPitch  = pitch_uv;
...
inline DP_STATUS_ENUM DpEngineBase::configFrameWrite(DpCommand &command, DpConfig &config)
{
    ...
    {
        DpColorFormat colorFormat = mdp_test::getInstance()->mapUnifiedColor(cfg.MDP_WROT_uniform_format[id], cfg.MDP_WROT_uniform_swap[id], 0, 0);
    ...
    {
        DpColorFormat colorFormat = mdp_test::getInstance()->mapUnifiedColor(cfg.MDP_WDMA_out_fmt[id], cfg.MDP_WDMA_swap[id], 0, 0);
