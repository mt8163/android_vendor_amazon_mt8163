...
DP_STATUS_ENUM DpEngine_TDSHP::onPostProc(DpCommand &command)
{
    ...
    if (pq_readback)
    {
        ...
        if (hdr_readback)
        {
            DPLOGI("DpEngine_TDSHP::onPostProc:hdr_readback\n");
            ...
            MM_REG_READ(command, MDP_HDR0_BASE + 0xE4, readbackPABuffer[((readbackPABufferIndex + counter) & (MAX_NUM_READBACK_PA_BUFFER - 1))], &m_regLabel[counter]);
            counter++;
#endif
        }
#endif
...
DP_STATUS_ENUM DpEngine_TDSHP::onReconfigTiles(DpCommand &command)
{
    ...
    if (pq_readback)
    {
        ...
        if (hdr_readback)
        {
            ...
            MM_REG_READ(command, MDP_HDR0_BASE + 0xE4, readbackPABuffer[((readbackPABufferIndex + counter) & (MAX_NUM_READBACK_PA_BUFFER - 1))], NULL, m_regLabel[counter]);
            counter++;
#endif
        }
#endif
