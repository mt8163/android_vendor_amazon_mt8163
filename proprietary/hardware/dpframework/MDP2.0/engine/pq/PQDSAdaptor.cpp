...
bool PQDSAdaptor::calRegs(PQSession* pPQSession, DpCommand &command, DpConfig &config)
{
    ...
    PQ_LOGI("[PQDSAdaptor] calRegs, time %d ms\n", diff);
...
void PQDSAdaptor::onCalcTDSHP(PQSession* pPQSession, DpCommand &command, DpConfig &config, bool enable)
{
    ...
    PQ_LOGI("[PQDSAdaptor] initDS Input, time %d ms\n", time_d);
    ...
    PQ_LOGI("[PQDSAdaptor] onCalculate, time %d ms\n", time_d);
    ...
    PQ_TIMER_GET_CURRENT_TIME(end);
    PQ_TIMER_GET_DURATION_IN_MS(begin, end, diff);
    PQ_LOGI("[PQDSAdaptor] onCalcTDSHP, time %d ms\n", diff);
}
                                ...
                                DpPqParam *PQParam)
{
    ...
    memcpy(&(inParam->iHWReg), &(dsConfig->TDSHP), sizeof(dsConfig->TDSHP));
#ifdef TDSHP_2_0
    {
...
void PQDSAdaptor::dumpFWReg(DpPqParam *PQParam, DSInput *input, DSOutput *output, DSReg *swReg)
{
    ...
    if (!enableDumpRegister && (PQParam->u.isp.p_mdpSetting == NULL || PQParam->u.isp.p_mdpSetting->buffer == NULL))
        return;

    if (PQParam->scenario == MEDIA_ISP_PREVIEW || PQParam->scenario == MEDIA_ISP_CAPTURE)
    {
        if (((enableDumpRegister == DUMP_ISP_PRV && PQParam->scenario == MEDIA_ISP_PREVIEW) ||
