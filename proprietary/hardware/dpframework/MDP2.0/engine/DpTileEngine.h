...
public:
    ...
    DP_STATUS_ENUM updatePMQOS(uint32_t& totalPixel, uint32_t& totalDataSzie, struct timeval& time)
    {
        DP_TRACE_CALL();
        DP_STATUS_ENUM status;
        ...
        {
            status = onUpdatePMQOS(totalPixel, totalDataSzie, time);
        }
...
protected:
    ...
    virtual DP_STATUS_ENUM onUpdatePMQOS(uint32_t& totalPixel __unused, uint32_t& totalDataSzie __unused, struct timeval& time __unused)
    {
        return DP_STATUS_RETURN_SUCCESS;
    }
