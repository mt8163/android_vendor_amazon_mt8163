...
#if defined(SUPPORT_HDR) && defined(HDR_IN_RDMA)
...
public:
    ...
    virtual int32_t getEngineFlag()
    {
#ifdef RDMA0_RSZ1_SRAM_SHARING
        if (getEngineType() == tRDMA0)
        {
            return (1 << tRDMA0) | (1 << tSCL1);
        }
#endif
...
#if defined(SUPPORT_HDR) && defined(HDR_IN_RDMA)
    bool getHDRInfo(DP_VDEC_DRV_COLORDESC_T *HDRInfo, uint64_t pqSessionId);
#endif
};
...
// Setup source frame info
    ...
    MM_REG_WRITE(command, MDP_RDMA_COMP_CON, (m_enableUFO  << 31), 0x80000000);
#endif
...
DP_STATUS_ENUM DpEngine_RDMA::onConfigTile(DpCommand &command)
{
    ...
    uint32_t       RDMA0_simple_mode    = m_useSimpleMode ? 1 : 0;
    ...
    MM_REG_WRITE(command, MDP_RDMA_CON, (RDMA0_lb_2b_mode  << 12) +
                                        (RDMA0_buffer_mode <<  8) +
                                        (RDMA0_simple_mode <<  4), 0x00001110);
...
DP_STATUS_ENUM DpEngine_RDMA::onUpdatePMQOS(uint32_t& totalPixel, uint32_t& totalDataSzie, struct timeval& endtime)
{
    uint32_t        plane = 0;
    uint32_t        data = 0;
