...
{
    if (m_ISPConfig.top.mdp_crop_en)
        MM_REG_WRITE_MASK(command, 0x15020030, 0x00000000, 0x00000200); // MDP_DL_SEL: select MDP_CROP for Cannon/Sylvia
    if (m_ISPConfig.top.mdp_crop2_en)
        MM_REG_WRITE_MASK(command, 0x15020030, 0x00000000, 0x00000C00); // MDP2_DL_SEL: select MDP_CROP2 for Cannon/Sylvia
                                           ...
                                           DpConfig  &config)
{
    ...
    if (m_ISPConfig.drvinfo.dip_secure_tag&0x1) // For secure camera path
    {
        if (m_ISPConfig.drvinfo.dip_img2o_secure_tag != 0) // IMG2O
        {
            secureRegAddr[0] = 0x15022330;
            secureRegAddr[1] = 0x15022360;
            secureRegAddr[2] = 0;
            planeOffset[0] = 0;
            planeOffset[1] = m_ISPConfig.drvinfo.dip_img2bo_base_addr;
            planeOffset[2] = 0;
            flag = (1<<CMDQ_ENG_ISP_IMG2O);
        ...
        if (m_ISPConfig.drvinfo.dip_pak2o_secure_tag != 0) // PAK2O
        {
            secureRegAddr[0] = 0x15022450;
            secureRegAddr[1] = 0;
            secureRegAddr[2] = 0;
            planeOffset[0] = 0;
            planeOffset[1] = 0;
            planeOffset[2] = 0;
            flag = (1<<CMDQ_ENG_ISP_IMG2O);
            command.secure(CMDQ_ENG_ISP_IMG2O, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.dip_pak2o_secure_tag, m_ISPConfig.drvinfo.dip_pak2o_base_addr, offset, m_ISPConfig.drvinfo.dip_pak2o_size, planeOffset);
        }
        if (m_ISPConfig.drvinfo.dip_imgi_secure_tag != 0) // IMGI (Separate secure setting for each plane due to BPCI and LSCI)
        {
            secureRegAddr[0] = 0x15022500;
            secureRegAddr[1] = 0;
            secureRegAddr[2] = 0;
            planeOffset[0] = 0;
            planeOffset[1] = 0;
            planeOffset[2] = 0;
            flag = (1<<CMDQ_ENG_ISP_IMGI);
            ...
            if(m_ISPConfig.top.bnr2_en == 1)
            {
                secureRegAddr[0] = 0x15022530;
                secureRegAddr[1] = 0;
                secureRegAddr[2] = 0;
                planeOffset[0] = 0;
                planeOffset[1] = 0;
                planeOffset[2] = 0;
                flag = (1<<CMDQ_ENG_ISP_IMGI);
                command.secure(CMDQ_ENG_ISP_IMGI, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.dip_imgbi_secure_tag, m_ISPConfig.drvinfo.dip_imgbi_base_addr, offset, m_ISPConfig.drvinfo.dip_imgbi_size, planeOffset);
            }
            else
            {
                secureRegAddr[0] = 0x15022530;
                ...
                planeOffset[2] = 0;
                flag = (1<<CMDQ_ENG_ISP_IMGI);
                command.secure(CMDQ_ENG_ISP_IMGI, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.dip_imgi_secure_tag, m_ISPConfig.drvinfo.dip_imgi_base_addr, offset, m_ISPConfig.drvinfo.dip_imgbi_size, planeOffset);
            }
            if(m_ISPConfig.top.lsc2_en == 1)
            {
                secureRegAddr[0] = 0x15022560;
                secureRegAddr[1] = 0;
                secureRegAddr[2] = 0;
                planeOffset[0] = 0;
                planeOffset[1] = 0;
                planeOffset[2] = 0;
                flag = (1<<CMDQ_ENG_ISP_IMGI);
                command.secure(CMDQ_ENG_ISP_IMGI, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.dip_imgci_secure_tag, m_ISPConfig.drvinfo.dip_imgci_base_addr, offset, m_ISPConfig.drvinfo.dip_imgci_size, planeOffset);
            }
            else
            {
                secureRegAddr[0] = 0x15022560;
                ...
                planeOffset[2] = 0;
                flag = (1<<CMDQ_ENG_ISP_IMGI);
                command.secure(CMDQ_ENG_ISP_IMGI, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.dip_imgi_secure_tag, m_ISPConfig.drvinfo.dip_imgi_base_addr, offset, m_ISPConfig.drvinfo.dip_imgci_size, planeOffset);
            }
        }
        if (m_ISPConfig.drvinfo.dip_vipi_secure_tag != 0) // VIPI
        {
            secureRegAddr[0] = 0x15022590;
            secureRegAddr[1] = 0x150225C0;
            secureRegAddr[2] = 0x150225F0;
            planeOffset[0] = 0;
            planeOffset[1] = m_ISPConfig.drvinfo.dip_vip2i_base_addr;
            planeOffset[2] = m_ISPConfig.drvinfo.dip_vip3i_base_addr;
            flag = (1<<CMDQ_ENG_ISP_VIPI);
        ...
        {
            secureRegAddr[0] = 0x15022620;
            secureRegAddr[1] = 0;
            secureRegAddr[2] = 0;
            planeOffset[0] = 0;
            planeOffset[1] = 0;
            planeOffset[2] = 0;
            flag = (1<<CMDQ_ENG_ISP_LCEI);
            command.secure(CMDQ_ENG_ISP_LCEI, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.dip_dmgi_secure_tag, m_ISPConfig.drvinfo.dip_dmgi_base_addr, offset, m_ISPConfig.drvinfo.dip_dmgi_size, planeOffset);
        }
        if (m_ISPConfig.drvinfo.dip_depi_secure_tag != 0) // DEPI
        {
            secureRegAddr[0] = 0x15022650;
            secureRegAddr[1] = 0;
            secureRegAddr[2] = 0;
            planeOffset[0] = 0;
            planeOffset[1] = 0;
            planeOffset[2] = 0;
            flag = (1<<CMDQ_ENG_ISP_LCEI);
            command.secure(CMDQ_ENG_ISP_LCEI, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.dip_depi_secure_tag, m_ISPConfig.drvinfo.dip_depi_base_addr, offset, m_ISPConfig.drvinfo.dip_depi_size, planeOffset);
        }
        if (m_ISPConfig.drvinfo.dip_lcei_secure_tag != 0) // LCEI
        {
            secureRegAddr[0] = 0x15022680;
            secureRegAddr[1] = 0;
            secureRegAddr[2] = 0;
            planeOffset[0] = 0;
            planeOffset[1] = 0;
        ...
        if (m_ISPConfig.drvinfo.dip_ufdi_secure_tag != 0) // UFDI
        {
            ...
            planeOffset[2] = 0;
            flag = (1<<CMDQ_ENG_ISP_LCEI);
            command.secure(CMDQ_ENG_ISP_LCEI, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.dip_ufdi_secure_tag, m_ISPConfig.drvinfo.dip_ufdi_base_addr, offset, m_ISPConfig.drvinfo.dip_ufdi_size, planeOffset);
        }
        // SMX1
        secureRegAddr[0] = 0x15022890;
        secureRegAddr[1] = 0;
        secureRegAddr[2] = 0;
        ...
        switch (m_ISPConfig.drvinfo.cqIdx) {
            case ISP_DRV_DIP_CQ_THRE0:
                ...
                command.secure(CMDQ_ENG_ISP_LCEI, flag, secureRegAddr, DP_SECURE, m_ISPConfig.drvinfo.cqSecHdl, offset, memSize, planeOffset);
                break;
            case ISP_DRV_DIP_CQ_THRE1:
                secureRegAddr[0] = 0x15022214;
                planeOffset[0] = m_ISPConfig.drvinfo.DesCqOft;
                memSize[0] = m_ISPConfig.drvinfo.cqSecSize - m_ISPConfig.drvinfo.DesCqOft;
                command.secure(CMDQ_ENG_ISP_LCEI, flag, secureRegAddr, DP_SECURE, m_ISPConfig.drvinfo.cqSecHdl, offset, memSize, planeOffset);
                break;
    ...
    MM_REG_WRITE_MASK(command, 0x15022004, m_ISPConfig.drvinfo.dip_ctl_yuv_en, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022008, m_ISPConfig.drvinfo.dip_ctl_yuv2_en, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x1502200C, m_ISPConfig.drvinfo.dip_ctl_rgb_en, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022010, m_ISPConfig.drvinfo.dip_ctl_dma_en, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022014, m_ISPConfig.drvinfo.dip_ctl_fmt_sel, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022018, m_ISPConfig.drvinfo.dip_ctl_path_sel, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x1502201C, m_ISPConfig.drvinfo.dip_ctl_misc_sel, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x150220E8, m_ISPConfig.drvinfo.dip_ctl_rgb2_en, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022330, m_ISPConfig.drvinfo.dip_img2o_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022360, m_ISPConfig.drvinfo.dip_img2bo_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022390, m_ISPConfig.drvinfo.dip_img3o_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x150223C0, m_ISPConfig.drvinfo.dip_img3bo_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x150223F0, m_ISPConfig.drvinfo.dip_img3co_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022420, m_ISPConfig.drvinfo.dip_feo_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022450, m_ISPConfig.drvinfo.dip_pak2o_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022500, m_ISPConfig.drvinfo.dip_imgi_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022530, m_ISPConfig.drvinfo.dip_imgbi_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022560, m_ISPConfig.drvinfo.dip_imgci_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022590, m_ISPConfig.drvinfo.dip_vipi_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x150225C0, m_ISPConfig.drvinfo.dip_vip2i_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x150225F0, m_ISPConfig.drvinfo.dip_vip3i_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022620, m_ISPConfig.drvinfo.dip_dmgi_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022650, m_ISPConfig.drvinfo.dip_depi_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022680, m_ISPConfig.drvinfo.dip_lcei_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x150226B0, m_ISPConfig.drvinfo.dip_ufdi_base_addr, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
                                           ...
                                           DpConfig  &config)
{
    ...
    // DIP settings for GCE cmd
    MM_REG_WRITE_MASK(command, 0x15022004, m_ISPConfig.drvinfo.dip_ctl_yuv_en, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022008, m_ISPConfig.drvinfo.dip_ctl_yuv2_en, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x1502200C, m_ISPConfig.drvinfo.dip_ctl_rgb_en, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022010, m_ISPConfig.drvinfo.dip_ctl_dma_en, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022014, m_ISPConfig.drvinfo.dip_ctl_fmt_sel, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022018, m_ISPConfig.drvinfo.dip_ctl_path_sel, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x1502201C, m_ISPConfig.drvinfo.dip_ctl_misc_sel, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x150220E8, m_ISPConfig.drvinfo.dip_ctl_rgb2_en, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022330, m_ISPConfig.drvinfo.dip_img2o_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022360, m_ISPConfig.drvinfo.dip_img2bo_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022390, m_ISPConfig.drvinfo.dip_img3o_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x150223C0, m_ISPConfig.drvinfo.dip_img3bo_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x150223F0, m_ISPConfig.drvinfo.dip_img3co_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022420, m_ISPConfig.drvinfo.dip_feo_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022450, m_ISPConfig.drvinfo.dip_pak2o_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022500, m_ISPConfig.drvinfo.dip_imgi_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022530, m_ISPConfig.drvinfo.dip_imgbi_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022560, m_ISPConfig.drvinfo.dip_imgci_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022590, m_ISPConfig.drvinfo.dip_vipi_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x150225C0, m_ISPConfig.drvinfo.dip_vip2i_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x150225F0, m_ISPConfig.drvinfo.dip_vip3i_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022620, m_ISPConfig.drvinfo.dip_dmgi_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022650, m_ISPConfig.drvinfo.dip_depi_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x15022680, m_ISPConfig.drvinfo.dip_lcei_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    MM_REG_WRITE_MASK(command, 0x150226B0, m_ISPConfig.drvinfo.dip_ufdi_base_addr, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
...
DP_STATUS_ENUM DpWrapper_ISP::onConfigTile(DpCommand &command)
{
    ...
    uint32_t       memSize[3] = {0};
    uint32_t       planeOffset[3] = {0};
    uint32_t       flag = (1<< CMDQ_ENG_ISP_IMGI);
    ...
    if (m_ISPConfig.drvinfo.dip_secure_tag&0x1)
    {
        ...
        command.secure(CMDQ_ENG_ISP_IMGI, flag, secureRegAddr, DP_SECURE, m_ISPConfig.drvinfo.tpipeTableSecHdl, offset, memSize, planeOffset);
    }
...
DP_STATUS_ENUM DpWrapper_ISP::onPostProc(DpCommand &command)
{
