                                           ...
                                           DpConfig  &config)
{
	...
	DPLOGI("wpe_secure_tag %d, wpe_wpei_secure_tag %d!\n", m_ISPConfig.drvinfo.wpe_secure_tag, m_ISPConfig.drvinfo.wpe_wpei_secure_tag);
	if(m_identifier == 0)
	{
		if (m_ISPConfig.drvinfo.wpe_secure_tag&0x1) // For secure camera path
		{
			if (m_ISPConfig.drvinfo.wpe_wpei_secure_tag != 0) // WPEI
			{
			    secureRegAddr[0] = WPE_ADDR_GEN_BASE_ADDR_0_HW;
			    secureRegAddr[1] = 0;
			    secureRegAddr[2] = 0;
			    planeOffset[0] = 0;
			    planeOffset[1] = 0;
			    planeOffset[2] = 0;
			    flag = (1<<CMDQ_ENG_WPEI);
			    command.secure(CMDQ_ENG_WPEI, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.wpe_wpei_secure_tag, m_ISPConfig.drvinfo.wpe_wpei_base_addr, offset, m_ISPConfig.drvinfo.wpe_wpei_size, planeOffset);

	            DPLOGI("flag %x, secureRegAddr %x!, m_ISPConfig.drvinfo.wpe_wpei_base_addr %x,  m_ISPConfig.drvinfo.wpe_wpei_size %x\n", flag, secureRegAddr, m_ISPConfig.drvinfo.wpe_wpei_base_addr, offset, m_ISPConfig.drvinfo.wpe_wpei_size);
				secureRegAddr[0] = WPE_ADDR_GEN_BASE_ADDR_1_HW;
			    command.secure(CMDQ_ENG_WPEI, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.wpe_wpei_secure_tag, m_ISPConfig.drvinfo.wpe_wpei_base_addr, offset, m_ISPConfig.drvinfo.wpe_wpei_size, planeOffset);
				secureRegAddr[0] = WPE_ADDR_GEN_BASE_ADDR_2_HW;
			    command.secure(CMDQ_ENG_WPEI, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.wpe_wpei_secure_tag, m_ISPConfig.drvinfo.wpe_wpei_base_addr, offset, m_ISPConfig.drvinfo.wpe_wpei_size, planeOffset);
				secureRegAddr[0] = WPE_ADDR_GEN_BASE_ADDR_3_HW;
			    command.secure(CMDQ_ENG_WPEI, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.wpe_wpei_secure_tag, m_ISPConfig.drvinfo.wpe_wpei_base_addr, offset, m_ISPConfig.drvinfo.wpe_wpei_size, planeOffset);
			}
			if (m_ISPConfig.drvinfo.wpe_wpeo_secure_tag != 0) // WPEO
			{
			    secureRegAddr[0] = WPE_WPEO_BASE_ADDR_HW;
			    secureRegAddr[1] = 0;
			    secureRegAddr[2] = 0;
			    planeOffset[0] = 0;
			    planeOffset[1] = 0;
			    planeOffset[2] = 0;
				flag = (1<<CMDQ_ENG_WPEO);
			    command.secure(CMDQ_ENG_WPEO, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.wpe_wpeo_secure_tag, m_ISPConfig.drvinfo.wpe_wpeo_base_addr, offset, m_ISPConfig.drvinfo.wpe_wpeo_size, planeOffset);
			}
		}
	...
	else
	{
	    if (m_ISPConfig.drvinfo.wpe_secure_tag&0x1) // For secure camera path
		{
			if (m_ISPConfig.drvinfo.wpe_wpei_secure_tag != 0) // WPEI
			{
				secureRegAddr[0] = WPE_B_ADDR_GEN_BASE_ADDR_0_HW;
			    secureRegAddr[1] = 0;
			    secureRegAddr[2] = 0;
			    planeOffset[0] = 0;
			    planeOffset[1] = 0;
			    planeOffset[2] = 0;
				flag = (1<<CMDQ_ENG_WPEI2);
			    command.secure(CMDQ_ENG_WPEI2, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.wpe_wpei_secure_tag, m_ISPConfig.drvinfo.wpe_wpei_base_addr, offset, m_ISPConfig.drvinfo.wpe_wpei_size, planeOffset);
				secureRegAddr[0] = WPE_B_ADDR_GEN_BASE_ADDR_1_HW;
			    command.secure(CMDQ_ENG_WPEI2, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.wpe_wpei_secure_tag, m_ISPConfig.drvinfo.wpe_wpei_base_addr, offset, m_ISPConfig.drvinfo.wpe_wpei_size, planeOffset);
				secureRegAddr[0] = WPE_B_ADDR_GEN_BASE_ADDR_2_HW;
			    command.secure(CMDQ_ENG_WPEI2, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.wpe_wpei_secure_tag, m_ISPConfig.drvinfo.wpe_wpei_base_addr, offset, m_ISPConfig.drvinfo.wpe_wpei_size, planeOffset);
				secureRegAddr[0] = WPE_B_ADDR_GEN_BASE_ADDR_3_HW;
			    command.secure(CMDQ_ENG_WPEI2, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.wpe_wpei_secure_tag, m_ISPConfig.drvinfo.wpe_wpei_base_addr, offset, m_ISPConfig.drvinfo.wpe_wpei_size, planeOffset);
			}
			if (m_ISPConfig.drvinfo.wpe_wpeo_secure_tag != 0) // WPEO
			{
				secureRegAddr[0] = WPE_B_WPEO_BASE_ADDR_HW;
			    secureRegAddr[1] = 0;
			    secureRegAddr[2] = 0;
			    planeOffset[0] = 0;
			    planeOffset[1] = 0;
			    planeOffset[2] = 0;
				flag = (1<<CMDQ_ENG_WPEO2);
			    command.secure(CMDQ_ENG_WPEO2, flag, secureRegAddr, (DpSecure)m_ISPConfig.drvinfo.wpe_wpeo_secure_tag, m_ISPConfig.drvinfo.wpe_wpeo_base_addr, offset, m_ISPConfig.drvinfo.wpe_wpeo_size, planeOffset);
			}
	    }
...
DP_STATUS_ENUM DpWrapper_WPE::onInitTileCalc(struct TILE_PARAM_STRUCT* p_tile_param)
{
    ...
    for (index = 0; index < ptr_tile_func_param->used_func_no; index++)
    {
        ...
        if (TILE_FUNC_WPE_ID == ptr_func->func_num)
        {
            m_pWPEDMA[WPE_TILE_FUNC_WPE] = ptr_func;
        }
...
DP_STATUS_ENUM DpWrapper_WPE::onRetrieveTileParam(struct TILE_PARAM_STRUCT* p_tile_param)
{
    ...
    if (ptr_tile_reg_map == NULL)
    {
        return DP_STATUS_UNKNOWN_ERROR;
    }
    //else //WPE frame mode case
    //{
    //    m_tileXEndFlag = 1;
    //    m_tileYEndFlag = 1;
	...
	for (int i = 0; i < WPE_TILE_FUNC_NUM; i++)
	{
		if(m_pWPEDMA[i]!=NULL)
		{
			switch(i)
			{
				case WPE_TILE_FUNC_WPE:
					m_inTileAccumulation += (m_pWPEDMA[i]->out_pos_xe - m_pWPEDMA[i]->out_pos_xs + 1);
					m_outFrameHeight = (m_pWPEDMA[i]->out_pos_ye - m_pWPEDMA[i]->out_pos_ys + 1);
					break;
				default:
					break;
			}
	...
	// Calculate WPE DMA BW
	if (ptr_tile_reg_map->VECI_EN)
		m_wpeDataSize += (ptr_tile_reg_map->VECI_XSIZE + 1) * (ptr_tile_reg_map->VECI_YSIZE + 1);
	if (ptr_tile_reg_map->VEC2I_EN)
		m_wpeDataSize += (ptr_tile_reg_map->VEC2I_XSIZE + 1) * (ptr_tile_reg_map->VEC2I_YSIZE + 1);
	if (ptr_tile_reg_map->VEC3I_EN)
		m_wpeDataSize += (ptr_tile_reg_map->VEC3I_XSIZE + 1) * (ptr_tile_reg_map->VEC3I_YSIZE + 1);
	if (ptr_tile_reg_map->WPEO_EN)
		m_wpeDataSize += (ptr_tile_reg_map->WPEO_XSIZE + 1) * (ptr_tile_reg_map->WPEO_YSIZE + 1);
	if (ptr_tile_reg_map->MSKO_EN)
		m_wpeDataSize += (ptr_tile_reg_map->MSKO_XSIZE + 1) * (ptr_tile_reg_map->MSKO_YSIZE + 1);
	if (ptr_tile_reg_map->WPE_EN)
		m_wpeDataSize += m_inTileAccumulation * m_outFrameHeight * 2  * 1.4 /*2 : yuv 422; 1.4 :cache miss*/;
...
DP_STATUS_ENUM DpWrapper_WPE::onUpdatePMQOS(uint32_t& totalPixel, uint32_t& totalDataSzie, struct timeval& endtime)
{
    uint32_t        pixelCount = 0;

    totalDataSzie += m_wpeDataSize;
    ...
    DPLOGI("DpWrapper_WPE: m_inTileAccumulation = %d\n", m_inTileAccumulation);
    DPLOGI("DpWrapper_WPE: m_outTileAccumulation = %d\n", m_outTileAccumulation);
    DPLOGI("DpWrapper_WPE: m_inFrameHeight = %d\n", m_inFrameHeight);
    DPLOGI("DpWrapper_WPE::updatePMQOS totalDataSzie = %d totalPixel = %d\n", totalDataSzie, totalPixel);
