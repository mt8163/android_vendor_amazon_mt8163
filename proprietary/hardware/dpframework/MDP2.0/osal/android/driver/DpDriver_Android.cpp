...
DP_STATUS_ENUM DpDriver::submitCommand(DpCommandBlock &block, DpJobID* pRet, uint32_t extRecorderFlag, char**)
{
    ...
    *pFrameInfo = block.getFrameInfoToCMDQ();
    ...
    DPLOGI("DpDriver: submit command block: start %#010llx, end %#010llx, engine flag %#010x\n",
        param.pVABase, param.pVABase + param.blockSize, param.engineFlag);
...
DP_STATUS_ENUM DpDriver::execCommand(DpCommandBlock &block)
{
    ...
    DPLOGI("DpDriver: exec command block: regAddresses: %#010llx, regValues: %#010llx, counter: %d\n",
        param.regRequest.regAddresses, param.regValue.regValues, param.regRequest.count);
    DPLOGI("DpDriver: exec command block: start %#010llx, end %#010llx, engine flag %#010x\n",
        param.pVABase, param.pVABase + param.blockSize, param.engineFlag);
    DPLOGI("DpDriver: exec command block: debugRegDump: %d\n", param.debugRegDump);
