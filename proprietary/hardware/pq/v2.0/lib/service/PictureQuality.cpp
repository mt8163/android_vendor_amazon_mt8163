...
Return<Result> PictureQuality::setSmartBacklightStrength(int32_t level) {
    ...
    if (level < MIN_SMART_BACKLIGHT_LEVEL || level > MAX_SMART_BACKLIGHT_LEVEL)
    {
        PQ_LOGE("[PQ_SERVICE] invalid parameter of level:[%d]!!", level);
        return Result::INVALID_ARGUMENTS;
    }

    ioctl(m_drvID, DISP_IOCTL_SET_SMARTBACKLIGHT, &level);
