...
#define DCRegOffset(field) (unsigned int)((char*)&( ((ADLReg*)(0))->field) - (char*)(0))
...
#else
    ALOGE("[PQDC_JNI] getIndex(), not supported!");
...
#else
    ALOGE("[PQDC_JNI] setIndex(), not supported!");
...
#else
    ALOGE("[PQDC_JNI] setTuningMode(), not supported!");
