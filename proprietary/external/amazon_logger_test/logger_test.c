...
static int print_log_msg(struct amazon_msg_buf* buf) {
    ...
    printf("sec[%d] nsec[%d] pid[%d] tid[%d] Pri[%c] tag[%s] msg[%s]\n", buf->entry.sec,
           buf->entry.nsec, buf->entry.pid, buf->entry.tid, log_pri_info[pri], tag, message);
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        fprintf(stderr,
                "Usage: logger_test [device_node of metrics/vitals]\n"
    ...
    while (count--) {
        ...
        if (retval == -1) {
            ...
            if ((ret > 0) && (ret != log_msg.entry.len + sizeof(struct logger_entry))) {
                fprintf(stderr,
                        "Error, read out length is not  expected as length=%d ="
